...
class SimDetectModel():
    ...
    def __init__(self, weights=sim_parse['weights'], sim_gpu_memeory=sim_parse['sim_gpu_memeory'],
                 device=sim_parse['device'], new_size=(640,640), stride=32):
        # self.g = tf.Graph()
...
if __name__ == '__main__':
    ...
    crop_store_path = './data/images'  # 裁剪的图片
    ...
    if not os.path.isdir(path):
        ...
        for img_name in img_name_list:
            ...
            with open(path + img_name, 'rb') as f:
                imgb64 = str(base64.b64encode(f.read()), 'utf-8')  # 转换成base64

            h, w, _ = img.shape

            img1 = cv2.resize(img, (640, 640))  # 这里转换成模型输入的大小
            boxes = sim_detect_model.pred(img1)
            print(boxes)  # [[35.7984619140625, 172.66712951660156, 579.2250366210938, 635.8843994140625, 0.5410427451133728, 0.0]]
            ...
            # 如果使用预标注，那么就执行这个功能
            if pre_label:
                make_prelabel(labeled_path, img_name, img, boxes, h, w, imgb64)
            else:
                # TODO 判断是否有检测到东西
                if len(boxes) == 0:

                    print('检测不到任何东西，请输入一张带有行人的照片')
                else:

                    # 否则就是裁剪图片然后进行后面的分类
                    for box in boxes:
                        if box[-1] == 0:
                            crop_imgs_list = crop_imgs(crop_store_path, img_name, img, box, is_save=is_save_cropped)
                            print(crop_imgs_list)
                            # 如果检测后需要后面的分类操作
